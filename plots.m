...
error_x = q_ref_odczyt.signals.values(:,2) - q_odczyt.signals.values(:,2);
error_y = q_ref_odczyt.signals.values(:,3) - q_odczyt.signals.values(:,3);
time = q_odczyt.time;
...
xlabel('Czas [s]');
ylabel('B³êdy');
legend('B³¹d theta [rad]', 'B³¹d x [m]', 'B³¹d y [m]');
...
ylabel('Pozycja oœ x');
legend('x [m]', 'x_{ref} [m]');
title('Porównanie po³o¿enia x rzeczywistego i x referencyjnego');
...
ylabel('Pozycja oœ y');
legend('y [m]', 'y_{ref} [m]');
title('Porównanie po³o¿enia y rzeczywistego i y referencyjnego');
%-----------------------------POMET--------------------------------------|
%%%%%%--- ea pomet
